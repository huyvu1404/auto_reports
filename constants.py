...
PROMPT_TEMPLATE = """
You are provided with a URL and a task. Your job is to complete the task using the provided URL.
You need to check the syntax of this URL: "{url}". If it is a valid URL, first check whether the URL or its content is accessible. If not, return an empty name with the status "Access Denied" and end the job.
...
+ Step 3: Once you have successfully logged in, wait for a popup to appear. Click "Close" or the "X" in the top-right corner to dismiss it.
...
+ Step 3: If the download is successful, return the name of the presentation file with the status "Downloaded successfully". Otherwise, return "Download failed" with an empty name.
